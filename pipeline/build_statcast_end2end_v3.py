...
PARQ      = f"{OUT}/statcast_ultra_full.parquet"      # 마스터 (존재 시 재사용)
...
def build_master_if_needed():
    ...
    if parq:
        ...
        union_sqls.append("SELECT rp.* FROM parq_files pf, read_parquet(pf.path) rp")
    if csv:
        con.execute("CREATE TEMP TABLE csv_files(path TEXT)")
        con.executemany("INSERT INTO csv_files VALUES (?)", [(p,) for p in csv])
        union_sqls.append("SELECT rc.* FROM csv_files cf, read_csv_auto(cf.path, HEADER=TRUE) rc")
    con.execute(f"COPY ({' UNION ALL '.join(union_sqls)}) TO '{q(PARQ)}' (FORMAT PARQUET)")
    con.close()
...
def create_people_view(con):
    ...
    if os.path.exists(chad):
        src = chad;  log("[MAP] using Chadwick people.csv")
    elif os.path.exists(lahm):
        src = lahm;  log("[MAP] using Lahman People.csv")
    else:
        log("⚠️ no people.csv; empty vw_map")
    ...
    mlbam_cols = [c for c in ["key_mlbam","mlbam","mlbam_id","key_mlbam_id"] if c in cols]
    first_cols = [c for c in ["nameFirst","name_first","first_name"] if c in cols]
    last_cols  = [c for c in ["nameLast","name_last","last_name"] if c in cols]
...
def build_seq_and_summaries_and_merge():
    con = duckdb.connect()
    con.execute("PRAGMA disable_progress_bar")
    con.execute("PRAGMA threads="+str(os.cpu_count() or 4))
    ...
    con.execute("""
        ...
        FROM agg a JOIN tot t ON a.season=t.season AND a.pitcher_id=t.pitcher_id
    """)
    ...
    con.execute("""
        ...
        joined AS (
          ...
          FROM pt_usage u JOIN used j ON u.season=j.season AND u.pitcher_id=j.pitcher_id
        ),
        rankd AS (
        ...
        SELECT
          ...
          rpt.repeat_rate
        FROM top1 ps
        FULL OUTER JOIN ent  ON ps.season=ent.season  AND ps.pitcher_id=ent.pitcher_id
        FULL OUTER JOIN rpt  ON COALESCE(ps.season,ent.season)=rpt.season
                             AND COALESCE(ps.pitcher_id,ent.pitcher_id)=rpt.pitcher_id
    """)
    ...
    # 카드 병합(있을 때만; OR-조인 제거 / 안전 조인)
    if os.path.exists(CARDS_ALL):
        log("[RUN] merge into enriched cards (ID + name fallback)")
        # rid 부여(조인 키)
        con.execute(f"CREATE OR REPLACE TABLE tmp_cards_base AS SELECT ROW_NUMBER() OVER () AS rid, * FROM read_csv_auto('{q(CARDS_ALL)}', HEADER=TRUE)")
        # retro -> mlbam 매핑
        con.execute("""
          CREATE OR REPLACE TABLE tmp_cards_idmap AS
          SELECT c.*, m.mlbam AS mlbam_map
          FROM tmp_cards_base c
          LEFT JOIN vw_map m ON lower(c.player_uid) = lower(m.retro)
        """)
        # 피처 요약 + 이름 보유
        con.execute("""
          ...
          LEFT JOIN id_name_pitcher p ON s.season=p.season AND s.pitcher_id=p.pitcher_id
        """)
        # ID 매칭
        con.execute("""
          CREATE OR REPLACE TABLE join_by_id AS
          SELECT c.rid,
                 ps.dominant_pitch AS dom_id,
                 ps.dominant_usage AS dom_usage_id,
                 ps.usage_entropy AS ent_id,
                 ps.repeat_rate   AS rep_id
          FROM tmp_cards_idmap c
          LEFT JOIN ps ON CAST(c.season AS INT)=ps.season
                      AND ps.pitcher_id = CAST(c.mlbam_map AS VARCHAR)
        """)
        # 이름 매칭
        con.execute("""
          CREATE OR REPLACE TABLE join_by_name AS
          SELECT c.rid,
                 ps.dominant_pitch AS dom_nm,
                 ps.dominant_usage AS dom_usage_nm,
                 ps.usage_entropy AS ent_nm,
                 ps.repeat_rate   AS rep_nm
          FROM tmp_cards_idmap c
          LEFT JOIN ps ON CAST(c.season AS INT)=ps.season
                      AND lower(ps.p_name) = lower(c.name)
        """)
        # 병합
        con.execute("""
          CREATE OR REPLACE TABLE tmp_cards_pitch AS
          SELECT c.*,
                 COALESCE(i.dom_id,  n.dom_nm)        AS dominant_pitch,
                 COALESCE(i.dom_usage_id, n.dom_usage_nm) AS dominant_usage,
                 COALESCE(i.ent_id, n.ent_nm)          AS usage_entropy,
                 COALESCE(i.rep_id, n.rep_nm)          AS repeat_rate
          FROM tmp_cards_idmap c
          LEFT JOIN join_by_id   i USING(rid)
          LEFT JOIN join_by_name n USING(rid)
        """)
        # 배터 각도(이름 매칭)
        con.execute("""
          CREATE OR REPLACE VIEW ba AS
        ...
        log(f"⚠️ {CARDS_ALL} not found; skip enrichment")

    cov = con.execute("SELECT MIN(season) AS min_y, MAX(season) AS max_y, COUNT(*) AS n_seasons FROM (SELECT DISTINCT season FROM sc)").fetchdf()
...
def main():
    open(LOG, "w").write(f"[{dt.datetime.now(dt.timezone.utc)}] end2end v3 start\n")
    build_master_if_needed()
    build_seq_and_summaries_and_merge()
    log("[DONE] end2end v3 complete")

if __name__ == "__main__":
    try:
        main(); sys.exit(0)
    except Exception as e:
        log(f"❌ ERROR: {e}"); sys.exit(0)
