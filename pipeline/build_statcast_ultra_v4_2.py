...
open(LOG,"w").write(f"[{dt.datetime.now(dt.timezone.utc)}] ULTRA v4.2 start\n")

def log(m): open(LOG,"a").write(m+"\n")

def q(path:str) -> str:
    # duckdb SQL literal quoting for paths
    return "'" + path.replace("'", "''") + "'"
...
def build_parts():
    ...
    for y in years:
        try:
            ...
            log(f"[RUN] part {y}: files={len(paths)} (pq={len(pqs)}, csv={len(csv)})")
            if pqs and csv:
...
def safe_copy(con, sql, path, fmt="CSV"):
    ...
    con.execute("COPY (" + sql + ") TO " + q(path) + f" (FORMAT {fmt})")
    ...
    if os.path.exists(pc) and who_col:
        try:
            ...
            duck = (
              ...
              "LEFT JOIN sc s ON CAST(p.player_uid AS VARCHAR)=CAST(" + who_col + " AS VARCHAR) "
              "GROUP BY ALL "
...
def make_report():
    ...
    try:
        ...
        if imgs:
            ...
            try:
                pdf = img2pdf.convert(*fhs)
                with open(f"{REP}/statcast_ultra_report.pdf","wb") as out: out.write(pdf)
...
def main():
    try:
        ...
        build_summaries()
        make_report()
        log("[DONE] ULTRA v4.2 complete")
        print("✅ Statcast ULTRA v4.2 Completed Successfully")
    except Exception as e:
        log(f"[FAIL] {e}")
